...
// Command to create table with desired column families
console.log("create 'italians', 'personal-data', 'app-data', 'professional-data', 'miscelaneous'")
...
for (let i = 0; i < 10000; ++i) {
    ...
    // Use Object class to create HBase shell commands
    Object.keys(mapping_dict).map(function(key, index) {
      
        // Get array with column names
        let column = mapping_dict[key];

        // Iterating over column names of the given key (column family)
        for (let j = 0; j <= column.length; j++){

            // Test if person has the given attribute
            if (typeof person[column[j]] != 'undefined'){
                
                let column_name = column[j];

                // Create HBase Shell cmd
                console.log(
                    'put',
                    JSON.stringify('italians'), ',',
                    JSON.stringify(person.username), ',',
                    JSON.stringify(key + ':' + column_name), ',',
                    JSON.stringify(person[column_name])
                );
